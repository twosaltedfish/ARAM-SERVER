...
// 每个流派自带的完整出装：核心装备 + 装备推荐合并为完整出装列表（与 coreGroups/recItems 同构）。
function buildFullItems(coreItems, situationalItems, itemNameById) {
...
function normalizeChampionDetail(raw, base = {}, itemNameById = {}) {
  ...
  const builds = buildsRaw.map((b, i) => {
    ...
    return {
      ...
      fullItems: buildFullItems(b.coreItems, b.situationalItems, itemNameById),
    };
  });
  ...
  return { champion, augments, coreGroups, augmentTrios, builds, recItems, fullItems: buildFullItems(firstBuild.coreItems, firstBuild.situationalItems, itemNameById), summonerSpells: raw.summonerSpells || [] };
}
