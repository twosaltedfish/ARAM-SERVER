...
// 根据 skillKeys 数组计算主/副加点技能：出现次数最多的为「主」，第二多的为「副」。
// 例：["Q","W","E","Q","Q","R","Q","E","Q","E","R","E","E","W","W"] → 主 Q · 副 E
function deriveMainSub(skillKeys) {
  const keys = (skillKeys || []).filter((k) => typeof k === 'string');
  if (!keys.length) return { main: '', sub: '' };
  const counts = {};
  keys.forEach((k) => {
    counts[k] = (counts[k] || 0) + 1;
  });
  const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]);
  const main = sorted[0] ? sorted[0][0] : '';
  const sub = sorted[1] ? sorted[1][0] : '';
  return { main, sub };
}
 ...
 * 限速——需要读取响应体判断：命中额度关键词(quota_exceeded/额度/余额/credits…)即按 QUOTA 处理（换 Key +
...
// 优先精确匹配第三方明确错误码 "quota_exceeded"；额度/余额等关键词作兜底。
// 注意需避开「rate limit exceeded」里的 exceed/out of 等限速常用词，否则会误判。
function looksLikeQuota(hint) {
  return /quota_exceeded|额度|余额|配额|credits?|insufficient|depleted|用完|不足/i.test(hint);
}
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        if (looksLikeQuota(hint)) {
          // 解析第三方结构化额度信息（error:"quota_exceeded" 体含 dailyCredits/usedCreditsToday），用于日志可读
          let used = '', total = '';
          try {
            const j = JSON.parse(hint);
            if (j && typeof j === 'object') {
              if (Number.isFinite(j.usedCreditsToday)) used = j.usedCreditsToday;
              if (Number.isFinite(j.dailyCredits)) total = j.dailyCredits;
            }
          } catch {}
          const extra = total !== '' ? ` (当日已用 ${used}/${total} credits)` : '';
          console.warn(`[dtodo] 429 命中额度耗尽 quota_exceeded，按 QUOTA 处理 key…${mask}${extra}`);
          throw Object.assign(new Error('QUOTA_EXCEEDED'), { code: 'QUOTA' });
        }
  ...
  return (coreItems || [])
    ...
    .map((grp, g) => {
      ...
      return {
        id: 'cg-' + g,
        items,
        winRate: num(grp.winRate) ? grp.winRate : 0,
...
// fullItems 源数据是「分组」结构（每组含 items[] + 组级别 winRate/pickRate）。
// 保持分组不展平，映射为与 coreGroups 同构：{ items:[{id,name,iconUrl}], winRateText, pickRateText }。
function buildFullItems(rawFullItems) {
  return (rawFullItems || [])
    .map((grp) => {
      const items = (grp.items || [])
        .filter((it) => it != null)
...
function normalizeChampionDetail(raw, base = {}, itemNameById = {}) {
  ...
  const augments = (raw.augments || [])
    .map((a) => {
      ...
      const augPy = pinyin(a.name || '', { toneType: 'none' }).replace(/\s/g, '');
      const augInitials = pinyin(a.name || '', { pattern: 'first', toneType: 'none' }).replace(/\s/g, '');
      return {
        _tier: tierVal,
        _wr: wr,
        _pr: pr,
  ...
  const builds = buildsRaw.map((b, i) => {
    ...
    return {
      ...
      fullItems: buildFullItems(b.fullItems),
      summonerSpells: b.summonerSpells || [],
      skillOrders: b.skillOrders || [],
      skillKeys: b.skillKeys || [],
      mainSub: deriveMainSub((b.skillOrders || [])[0] ? (b.skillOrders[0].skillKeys || []) : []),
    };
  });
  ...
  return { champion, augments, coreGroups, augmentTrios, builds, recItems, fullItems: raw.fullItems || [], summonerSpells: raw.summonerSpells || [], skillOrders: (builds[0] && builds[0].skillOrders) || [], skillKeys: (builds[0] && builds[0].skillKeys) || [], mainSub: (builds[0] && builds[0].mainSub) ? builds[0].mainSub : {} };
}
