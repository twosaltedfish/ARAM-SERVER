...
// 缓存：每条详情记录 updatedAt 日期，若当日已拉取过则自动跳过（FORCE=true 强制重拉），
// 因此同日内重复运行 sync 几乎不消耗 credits，仅跨天或修复解析 bug 时才重拉。
...
// ===== 日期缓存工具 =====
// 每次成功拉取的数据都打上 updatedAt（YYYY-MM-DD）；若 updatedAt 为当天且非 FORCE，
// 则跳过该数据，避免同日内重复消耗 credits。
function todayStr() {
