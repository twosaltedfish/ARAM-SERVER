 ...
 * Key，采用「顺序耗尽」策略——按配置顺序逐 Key 尝试，某 Key 额度耗尽(402)或限速(429)都自动
 * 切换到下一个 Key；不做轮询/负载均衡；所有 Key 都不可用则抛错。
...
async function fetchJson(apiPath, { apiKey, retries = 4, timeoutMs = 20000 } = {}) {
  ...
  // - 某 Key 额度耗尽(402/QUOTA)：标记 exhausted 并切换下一 Key（顺序耗尽策略）
  // - 某 Key 限速(429/RATELIMIT)：本题修复点——切换下一 Key 继续，而非死磕原 Key
  ...
  for (const key of KEY_POOL) {
    if (keyState.get(key).exhausted) continue;
    try {
      ...
      if (e.code === 'QUOTA') {
        ...
        console.warn(`[dtodo] Key…${key.slice(-4)} 限速，切换下一 Key 继续`);
      } else {
