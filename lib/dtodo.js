 ...
 * Key，采用「顺序耗尽」策略——优先用第一个未耗尽的 Key，用完（402 额度耗尽）再切下一个，
 * 不做轮询/负载均衡；所有 Key 耗尽则抛 QUOTA_EXCEEDED。
...
// 选 Key：顺序耗尽策略——优先用第一个未耗尽的 Key，用完（402）再切下一个，不轮询/不均衡
function pickKey() {
...
const MIN_INTERVAL_MS = 2000; // 每个成功请求后强制最小间隔，避免突发请求触发网关超时
