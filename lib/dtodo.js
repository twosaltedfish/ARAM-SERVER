 ...
 * Key，请求在 Key 间自动负载均衡（按响应头 x-credits-remaining 选剩余额度最高者），
 * 某 Key 额度耗尽(402)自动切换下一个未耗尽 Key；所有 Key 耗尽则抛 QUOTA_EXCEEDED。
...
// 选 Key：优先剩余额度高且未耗尽的 Key；额度未知时在未耗尽 Key 间轮询，避免总打同一个
function pickKey() {
  resetExhaustedIfNewDay();
  const candidates = KEY_POOL.filter((k) => !keyState.get(k).exhausted);
  if (!candidates.length) return null;
  const rank = (k) => {
    const r = keyState.get(k).remaining;
    return r === Infinity ? 1e12 : r;
  };
  candidates.sort((a, b) => rank(b) - rank(a));
  const topRemaining = rank(candidates[0]);
  const top = candidates.filter((k) => rank(k) === topRemaining);
  const chosen = top[keyCursor % top.length];
  keyCursor++;
  return chosen;
}
...
// 单次请求（绑定单个 Key）：含超时/429 重试；402 抛 QUOTA(code) 由上层决定是否换 Key
async function fetchWithKey(url, apiKey, { retries = 4, timeoutMs = 20000, trackHeader = false } = {}) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    if (res.status === 402) {
      // 当日 credits 额度耗尽，抛特殊错误让上层换 Key 或停止
      throw Object.assign(new Error('QUOTA_EXCEEDED'), { code: 'QUOTA' });
    }
    if (res.status === 429) {
      const wait = 5000 * attempt;
      console.warn(`[dtodo] 429 限速，key…${mask} ${attempt}/${retries} 次重试，等待 ${wait}ms`);
      await sleep(wait);
      continue;
    }
    if (!res.ok) {
...
async function fetchJson(apiPath, { apiKey, retries = 4, timeoutMs = 20000 } = {}) {
  ...
  // 显式传入 Key（脚本指定）→ 单 Key 模式，直接走 fetchWithKey
  if (explicitKey) {
    return fetchWithKey(url, explicitKey, { retries, timeoutMs, trackHeader: true });
  }

  // 多 Key 模式：逐 Key 尝试，某 Key 额度耗尽(402)自动切换下一个未耗尽 Key
  if (!KEY_POOL.length) {
    throw new Error('NO_API_KEY_CONFIGURED');
  }
  let lastErr;
  for (let guard = 0; guard < KEY_POOL.length; guard++) {
    const key = pickKey();
    if (!key) break; // 所有 Key 当天耗尽
    try {
      ...
      throw e; // 网络/HTTP 非 402 错误直接抛出，不换 Key（避免误烧其他 Key 额度）
    }
  }
  throw lastErr || new Error(`dtodo ${apiPath} 所有 Key 均不可用`);
}
